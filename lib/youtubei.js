...
async function searchAndDownload(query, outputPath, type = 'video') {
  try {
    ...
    if (type === 'video') {
      ...
      throw new Error('Type : usage "video" ou "audio".');
    }
  } catch (error) {
...
async function downloadYouTubeVideo(url, outputPath) {
  try {
    ...
    if (!videoFormat || !videoFormat.url) throw new Error('Aucun format vidéo trouvé');
    const response = await fetch(videoFormat.url);
    if (!response.ok) throw new Error('Erreur');
    const totalSize = parseInt(response.headers.get('content-length'), 10);
    if (!totalSize) throw new Error('Erreur de longueur de contenu');
...
async function downloadYouTubeAudio(url, outputPath) {
  try {
    ...
    if (!audioFormat || !audioFormat.url) throw new Error('Aucun format audio trouvé');
    const response = await fetch(audioFormat.url);
    if (!response.ok) throw new Error('Erreur');
    const totalSize = parseInt(response.headers.get('content-length'), 10);
    if (!totalSize) throw new Error('Erreur');
    console.log(`${url}`);
    const fileStream = fs.createWriteStream(outputPath);
    let downloaded = 0;
    response.body.on('data', chunk => {
...
async function mixAudioAndVideo(videoPath, audioPath, outputPath) {
  return new Promise((resolve, reject) => {
    ffmpeg()
      ...
      .on('end', () => {
        console.log('Audio et vidéo combinés avec succès');
        resolve();
      })
      .on('error', (err) => {
...
async function combineYouTubeVideoAndAudio(videoQuery, audioQuery, outputPath) {
  try {
    ...
    if (!videoResults || videoResults.length === 0) throw new Error('Aucun résultat');
    if (!audioResults || audioResults.length === 0) throw new Error('Aucun résultat');
